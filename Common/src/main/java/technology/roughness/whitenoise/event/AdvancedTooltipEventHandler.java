            ...
            if (ConfigHandler.Client.showAdvancedTooltips() && Minecraft.getInstance().options.advancedItemTooltips) {
                ...
                if (Screen.hasControlDown()) {
                    for (TypedDataComponent<?> typedDataComponent : itemStack.getComponents()) {
                        ...
                        if (value instanceof ItemLore(List<Component> lines, List<Component> styledLines)) {
                            showComponent = !lines.isEmpty() || !styledLines.isEmpty();
                        }
                        else if (value instanceof ItemAttributeModifiers modifiers) {
                            showComponent = !modifiers.modifiers().isEmpty();
                        }
                        else if (value instanceof ItemEnchantments itemEnchantments) {
                            showComponent = !itemEnchantments.keySet().isEmpty();
                        }
                        else if (value instanceof ItemContainerContents) {
                            showComponent = false;
                        }
